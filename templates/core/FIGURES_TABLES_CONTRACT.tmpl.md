...
## 0) Figure Generation Rule (Non-Negotiable)

> **All figures must be generated by scripts from raw JSON outputs. No hardcoded values. No manual figure creation.**
>
> Every figure must have:
> - A generating script in scripts/ (e.g., make_report_artifacts.py)
> - Input: one or more files in outputs/
> - Output: file in figures/
> - Deterministic: same inputs → same figure
>
> **Mandatory figure types for ML projects:**
> 1. Learning curves (from LEARNING_CURVE_SPEC)
> 2. Model complexity curves (from MODEL_COMPLEXITY_SPEC)
> 3. Confusion matrices (if classification)
> 4. SHAP/feature importance (if explainability required)
